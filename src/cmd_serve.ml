...
let log_startup c env =
  let app_dir = Hyperbib_app.Conf.app_dir (Service_env.conf env) in
...
let setup_service ~conf ~service_path ~secure_cookie ~env =
  let pk_file = Hyperbib_app.Conf.authentication_private_key conf in
...
let start_backup_thread ~conf ~db_pool ~backup_every_s =
  ...
  | None -> ()
  | Some every_s ->
      let backup = Hyperbib_app.Conf.db_backup_file conf in
...
let cmd =
  ...
  in
  Hyperbib_app.Cli.cmd_with_conf "serve" ~doc ~man @@
