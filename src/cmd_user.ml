...
let add ~name ~password ~force conf =
  ...
  let users_file = Hyperbib_app.Conf.users_file conf in
  ...
  | true when not force ->
      ...
      Ok Hyperbib_app.Exit.user_exists
  | _ ->
...
let add_cmd =
  ...
  in
  Hyperbib_app.Cli.cmd_with_conf "add" ~doc ~man @@
...
let delete_cmd =
  ...
  in
  Hyperbib_app.Cli.cmd_with_conf "delete" ~doc ~man @@
...
let list_cmd =
  ...
  in
  Hyperbib_app.Cli.cmd_with_conf "list" ~doc ~man @@
  Term.(const list)

let cmd =
  let doc = "Manage application users" in
  Hyperbib_app.Cli.cmd_group "user" ~doc @@
  [add_cmd; list_cmd]
