...
let ui_ext g ~self =
  ...
  let new_button =
    ...
    Adhoc_html.new_entity_button ~href ~label:Uimsg.new_subject
  in
...
let h1_subject uf ~self ?name s =
  let entity_kind =
    ...
    Adhoc_html.entity_kind_index uf ~self ~kind (Subject.Url.v Index)
  in
...
let new_form g s ~parents ~cancel =
  ...
  let title = Adhoc_html.title ~sub:Uimsg.new_subject ~sup:Uimsg.subject in
  let content =
    let at = At.[Hclass.entity; Hclass.editing] in
...
| None -> El.void
| Some p ->
    let link = Adhoc_html.link_subject uf ~self p in
...
let edit_ui g uf s =
  ...
  let edit = Adhoc_html.htmlact_edit_button uf (Subject.Url.v (Edit_form sid)) in
  let rep =
    Adhoc_html.htmlact_replace_button uf (Subject.Url.v (Replace_form sid))
  in
  let dup =
    Adhoc_html.htmlact_duplicate_button uf (Subject.Url.v (Duplicate_form sid))
  in
  let del =
    Adhoc_html.htmlact_delete_button uf (Subject.Url.v (Confirm_delete sid))
  in
...
let index_html g ~self ss ~ref_count =
  ...
  let subject s = Adhoc_html.link_subject uf ~self s in
  let ref_count s = match Subject.Id.Map.find_opt (Subject.id s) ref_count with
  | None -> 0 | Some (_, c) -> c
  in
  let child s =
    let sid = anchor_id s in
    let count = Adhoc_html.item_count (ref_count s) in
    El.li ~at:At.[id sid] [Adhoc_html.anchor_a sid; subject s; El.sp; count]
  in
  let parent children r =
    let h2 =
      let sid = anchor_id r in
      let count = Adhoc_html.item_count (ref_count r) in
      El.h2 ~at:At.[id sid] [Adhoc_html.anchor_a sid; subject r; El.sp; count]
    in
  ...
  let h1 =
    ...
    El.h1 [Adhoc_html.uppercase_span Uimsg.subjects; El.sp; count ]
  in
  let tree =
