...
let link_bibtex_file g uf ~self =
  ...
  Adhoc_html.link ~href (El.splice [El.code [El.txt ".bib"]; El.txt " file"])
...
let link_suggest_page g uf ~self =
  ...
  in
  Adhoc_html.link ~href (El.txt txt)
...
let user_login uf ~self =
  ...
  in
  El.span ~at:At.[Hclass.user_ui] [Adhoc_html.link ~href (El.txt Uimsg.login)]
...
let user_view g uf ~self view =
  let pub =
    ...
    | `Private ->
        let req =
          Adhoc_html.htmlact_request uf (User.Url.v (View {private' = false}))
        in
  ...
  let priv =
    ...
    | `Public ->
        let req =
          Adhoc_html.htmlact_request uf (User.Url.v (View {private' = true}))
        in
        [req; Htmlact.effect' `None]
    in
...
let user_view_ui ?(ui_ext = fun _ ~self:_ -> El.void) g uf ~self =
  ...
  | Some view ->
      ...
      | `Private ->
          ...
          let link = El.span [Adhoc_html.link ~href (El.txt Uimsg.labels)] in
...
let ui ?ui_ext g ~self =
  ...
  let page_header =
    ...
    El.header [
      El.h1 [Adhoc_html.link ~href (El.txt short_title)];
...
let full_title g ~title =
  ...
  if title = proj then proj else Adhoc_html.title ~sub:title ~sup:proj
...
let footer g =
  ...
  let link_project =
    ...
    Adhoc_html.link ~href (El.txt_of Bibliography.project_title b)
  in
  El.footer
...
let for_404 ?ui_ext g ~kind ~self ~consult =
  let uf = Gen.url_fmt g in
  let kind_uncap = Adhoc_html.uncapitalize kind in
  ...
  let content =
    El.section [
      ...
      El.p [Adhoc_html.link ~href:consult_href (El.txt consult_text)]]
  in
...
let error g request response' =
  ...
  let goto_bib = Adhoc_html.link ~href:bib_url (El.txt Uimsg.go_back_to_bib) in
  let content =
