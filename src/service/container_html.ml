...
let ui_ext g ~self =
  ...
  let new_button =
    ...
    Adhoc_html.new_entity_button ~href ~label:Uimsg.new_container
  in
...
let h1_container ?title uf ~self c =
  ...
  let entity_kind =
    ...
    Adhoc_html.entity_kind_index uf ~self ~kind (Container.Url.v Index)
  in
...
let deleted g c =
  let cont = Adhoc_html.uncapitalize Uimsg.container in
  let goto = Container.Url.v Index in
  let goto = Kurl.Fmt.url (Page.Gen.url_fmt g) goto in
  let goto = Adhoc_html.link ~href:goto (El.txt_of Uimsg.goto_kind_index cont) in
...
let new_form g c ~cancel =
  ...
  let title = Adhoc_html.title ~sub:Uimsg.new_container ~sup:Uimsg.container in
  let content =
    let at = At.[Hclass.entity; Hclass.editing] in
...
let replace_form g c ~ref_count =
  ...
  let at =
    let r =
      Adhoc_html.htmlact_request uf (Container.Url.v (Replace (Container.id c)))
    in
...
let edit_ui g uf c =
  ...
  let edit =
    Adhoc_html.htmlact_edit_button uf (Container.Url.v (Edit_form cid)) in
  let rep =
    Adhoc_html.htmlact_replace_button uf (Container.Url.v (Replace_form cid)) in
  let dup =
    Adhoc_html.htmlact_duplicate_button uf (Container.Url.v (Duplicate_form cid))
  in
  let del =
    Adhoc_html.htmlact_delete_button uf (Container.Url.v (Confirm_delete cid))
  in
...
let page_title c = Adhoc_html.title ~sub:(Container.title c) ~sup:Uimsg.container
...
let index_html g ~self cs ~ref_count =
  ...
  let container_li p =
    ...
    let cont = Adhoc_html.link_container uf ~self p in
    let count = Adhoc_html.item_count (ref_count p) in
    let container = [Adhoc_html.anchor_a cid; cont; El.sp; count] in
    El.li ~at:At.[id cid] container
  in
  let letter_section (l, cs) =
    let cs = List.sort Container.order_by_title cs in
    El.splice [Adhoc_html.h2_letter l; El.ol (List.map container_li cs)]
  in
  let h1 =
    let count = Adhoc_html.item_count (List.length cs) in
    El.h1 [Adhoc_html.uppercase_span Uimsg.containers; El.sp; count ]
  in
  let descr = Adhoc_html.description (El.txt Uimsg.container_list_descr) in
  let index =
