...
let input_container ~name ~containers:cs =
  (* XXX this doesn't scale. *)
  let cs = List.sort Container.order_by_title cs in
  let option c =
    let at = [At.value (Res.Id.to_string (Container.id c))] in
    El.option ~at [El.txt_of Container.title c]
  in
  let at = [Hclass.container; Hui.Class.input; Hclass.select;
            At.required; At.name name] in
  El.select ~at (List.map option cs)

let replace_form g c ~ref_count ~containers =
  let self = Container.Url.page c in
  let uf = Page.Gen.url_fmt g in
  let h1 = h1_container uf ~self c in
  let buttons =
  ...
  in
  El.form ~at [h1; intro; El.div [input_container]; msg; buttons]
