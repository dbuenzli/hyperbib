...
let resp_err_doi_not_found g ~self doi = (* move to reference_html *)
  ...
  Reference_html.filled_in_form g
    Reference.new' ~self ~msg ~authors:[] ~editors:[] ~container:None
    ~cites:[]

let resp_err_doi_error g ~self = (* move to reference_html *)
  (* FIXME it would nice to preserve the form user input here *)
  let at = [Hclass.message; Hclass.error] in
  let msg = El.p ~at [El.txt Uimsg.doi_error]in
  Reference_html.filled_in_form g
    Reference.new'
    ~self ~msg ~authors:[] ~editors:[] ~container:None ~cites:[]
...
let fill_in_form app doi =
  let* () = Entity_service.check_edit_authorized app in
  Webapp.with_db_transaction' `Deferred app @@ fun db ->
  let self (* XXX *) = Reference.Url.v (New_form { cancel = None }) in
  let doi_cache = Hyperbib.Data_conf.doi_cache_dir (Webapp.data_conf app) in
  Result.map_error
    (* Bof *)
    (fun e -> Result.get_error (Resp.server_error_500 ~explain:e ())) @@
  let g = Webapp.page_gen app in
  ...
  match ref with
  | Error e -> Ok (Page.resp_part ~explain:e (resp_err_doi_error g ~self))
  | Ok None -> Ok (Page.resp_part (resp_err_doi_not_found g ~self doi))
  | Ok (Some ref) ->
      let r =
      ...
      let html =
        Reference_html.filled_in_form g r
...
| Fill_in_form doi -> fill_in_form app doi
| Index -> index app
